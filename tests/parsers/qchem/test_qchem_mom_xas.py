...
def test_mom_xas_job1_scf_results(parsed_mom_smd_xas_data: MomCalculationResult) -> None:
    ...
    assert job1_data.scf.energy == pytest.approx(-76.53682182, abs=ENERGY_TOL)  # E_SCF (including G_PCM)
    assert job1_data.final_energy == pytest.approx(-76.53447435, abs=ENERGY_TOL)  # G(tot) = G_ENP + G_CDS

    last_iter_job1 = job1_data.scf.iterations[-1]
    assert last_iter_job1.iteration == 7
    assert last_iter_job1.energy == pytest.approx(-76.5368218181, abs=ENERGY_TOL)
    ...
    assert job1_data.smd.g_pcm_kcal_mol == pytest.approx(-11.0106, abs=MULTIPOLE_TOL)
    assert job1_data.smd.g_cds_kcal_mol == pytest.approx(1.4731, abs=MULTIPOLE_TOL)
    assert job1_data.smd.g_enp_au == pytest.approx(-76.53682182, abs=ENERGY_TOL)
    assert job1_data.smd.g_tot_au == pytest.approx(-76.53447435, abs=ENERGY_TOL)
...
def test_mom_xas_job2_scf_results(parsed_mom_smd_xas_data: MomCalculationResult) -> None:
    ...
    assert job2_data.scf.energy == pytest.approx(-76.53682225, abs=ENERGY_TOL)
    assert job2_data.final_energy == pytest.approx(-76.53447478, abs=ENERGY_TOL)

    assert job2_data.smd is not None, "SMD results missing in Job 2"
    assert job2_data.smd.g_pcm_kcal_mol == pytest.approx(-11.0110, abs=MULTIPOLE_TOL)
    assert job2_data.smd.g_cds_kcal_mol == pytest.approx(1.4731, abs=MULTIPOLE_TOL)
    assert job2_data.smd.g_enp_au == pytest.approx(-76.53682225, abs=ENERGY_TOL)
    assert job2_data.smd.g_tot_au == pytest.approx(-76.53447478, abs=ENERGY_TOL)
...
def test_mom_xas_job2_scf_iterations(parsed_mom_smd_xas_data: MomCalculationResult) -> None:
    ...
    expected_iterations_data = [
        (1, -76.5192748968, 7.73e-04, "Roothaan", True, "IMOM", 5.0, 5.0),
        (1, -76.5328144078, 1.03e-03, None, True, "IMOM", 5.00, 5.0),
        (2, -76.5346554633, 1.11e-03, None, True, "IMOM", 5.00, 5.0),
        (3, -76.5367667999, 1.55e-04, None, True, "IMOM", 5.00, 5.0),
...
def test_mom_xas_job2_tddft_tda_states(parsed_mom_smd_xas_data: MomCalculationResult) -> None:
    ...
    assert tda_es1.excitation_energy_ev == pytest.approx(533.9299, abs=EXCITATION_ENERGY_TOL)
    assert tda_es1.total_energy_au == pytest.approx(-56.91526182, abs=ENERGY_TOL)
    ...
    assert tda_es3.oscillator_strength == pytest.approx(0.0167642453, abs=OS_STRENGTH_TOL)
...
def test_mom_xas_job2_excited_state_analysis(parsed_mom_smd_xas_data: MomCalculationResult) -> None:
    ...
    assert es1_analysis.multipole.dipole_moment_debye.magnitude == pytest.approx(0.854466, abs=MULTIPOLE_TOL)
...
def test_mom_xas_job2_transition_density_matrix_analysis(parsed_mom_smd_xas_data: MomCalculationResult) -> None:
    ...
    assert tdm1.mulliken is not None
    assert len(tdm1.mulliken.populations) == 3
    assert tdm1.mulliken.populations[0].symbol == "H"
    assert tdm1.mulliken.populations[0].transition_charge_e == pytest.approx(-0.000000, abs=POPULATION_TOL)
    assert tdm1.mulliken.populations[0].hole_charge_alpha_uks == pytest.approx(0.000072, abs=POPULATION_TOL)
    assert tdm1.mulliken.populations[0].electron_charge_beta_uks == pytest.approx(-0.224138, abs=POPULATION_TOL)

    assert tdm1.ct_numbers is not None
    assert tdm1.ct_numbers.omega == pytest.approx(1.0000, abs=POPULATION_TOL)
    assert tdm1.ct_numbers.loc_a == pytest.approx(0.2383, abs=POPULATION_TOL)
    ...
    assert tdm3.mulliken is not None
    assert tdm3.mulliken.populations[1].symbol == "O"
    assert tdm3.mulliken.populations[1].transition_charge_e == pytest.approx(0.001113, abs=POPULATION_TOL)
