...
def test_qchem_54_mom_initial_scf_job_results(parsed_mom_smd_sp_data: MomCalculationResult) -> None:
    ...
    assert last_iter_job1.energy == pytest.approx(
        -76.4562213331, abs=ENERGY_TOL
    )  # Note: energy here might be before final basis set correction if parsed this way
    ...
    assert job1_data.smd is not None
    print(repr(job1_data.smd))
...
def test_qchem_54_mom_driven_scf_job_results(parsed_mom_smd_sp_data: MomCalculationResult) -> None:
    ...
    assert job2_data.scf.energy == pytest.approx(
        -76.1676313769, abs=ENERGY_TOL
    )  # Final energy after basis set correction
...
def test_qchem_54_specific_mom_scf_iterations_job2(parsed_mom_smd_sp_data: MomCalculationResult) -> None:
    ...
    expected_iterations_data = [
        ...
        (1, -76.0833919817, 7.23e-03, True, "IMOM", 5.0, 5.0, "Roothaan"),  # First iteration (Roothaan)
        (1, -76.1266046234, 6.45e-03, True, "IMOM", 4.99, 5.0, None),  # First DIIS, Iter 1 in output table
        (2, -76.1420331889, 5.03e-03, True, "IMOM", 4.98, 5.0, None),  # Second DIIS, Iter 2
        (3, -76.1698796165, 1.63e-04, True, "IMOM", 4.98, 5.0, None),  # Third DIIS, Iter 3
        (4, -76.1700093835, 7.72e-05, True, "IMOM", 4.98, 5.0, None),  # Fourth DIIS, Iter 4
        (5, -76.1706945744, 2.70e-04, True, "IMOM", 4.98, 5.0, None),  # Fifth DIIS, Iter 5
        (6, -76.1699784580, 1.19e-05, True, "IMOM", 4.98, 5.0, None),  # Sixth DIIS, Iter 6
        (7, -76.1699788412, 3.44e-06, None, None, None, None, None),  # Seventh DIIS, Iter 7 (Converged)
    ]
